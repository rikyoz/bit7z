...
TEST_CASE( "CMultiVolumeInStream: Seeking with an invalid origin", "[cmultivolumeinstream][seeking]" ) {
    // A non-existent first volume yields a stream with no volumes; Seek validates the origin
    // and returns before accessing any volume, so no on-disk fixture is needed here.
    CMultiVolumeInStream inStream{ "non_existent_volume.001" };
