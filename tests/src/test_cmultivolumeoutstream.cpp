...
using bit7z::CMultiVolumeOutStream;

TEST_CASE( "CMultiVolumeOutStream: Seeking with an invalid origin", "[cmultivolumeoutstream][seeking]" ) {
    // The constructor only stores the volume size and name prefix; Seek validates the origin and
    // returns before creating or writing any volume, so no on-disk fixture is needed here.
    CMultiVolumeOutStream outStream{ 1024, "non_existent_archive" };
