...
using is_char = std::integral_constant<
    bool,
...
TEST_CASE( "util: Narrowing wide string to std::string", "[stringutil][narrow]" ) {
    ...
    SECTION( "Converting wide strings with multiple UTF-16 codeunits invalid in UTF-8" ) {
        const std::wstring testInput = GENERATE(
            ...
            // Two low surrogates.
            L"\xDE02\xDE02",
            // Only high surrogates.
            L"\xD800\xD83D\xDA80\xDBFF",
            // Only low surrogates.
            L"\xDC00\xDC80\xDE02\xDFFF",
            // Low surrogate before a high surrogate.
            L"\xDE02\xD83D",
            L"\xDFFF\xD800",
            // Multiple out-of-range Unicode characters.
            L"\x110000\x200000",
            L"\x200000\x300000\x400000",
            L"\x300000\x400000\x500000\x600000",
            // Mixed lone surrogate and out-of-range Unicode character.
            L"\xD83D\x200000",
    ...
    SECTION( "Converting wide strings to UTF-8 (no surrogate pairs)" ) {
        ...
        std::tie( testInput, testOutput ) = GENERATE(
            table< const wchar_t*, const char* >(
                {
                    NARROWING_TEST_STR( "" ),
                    ...
                    NARROWING_TEST_STR( "\u2010" ),
                    // 丈
                    NARROWING_TEST_STR( "\u4E08" ),
                    // 世界
                    NARROWING_TEST_STR( "\u4E16\u754C" ),
                    NARROWING_TEST_STR( "\uE000" ),
                    // Replacement character U+FFFD
                    NARROWING_TEST_STR( "\uFFFD" ),
                    // メタルガルルモン
                    NARROWING_TEST_STR( "\u30e1\u30bf\u30eb\u30ac\u30eb\u30eb\u30e2\u30f3" ),
            ...
            table< const wchar_t*, const char* >(
                {
                    ...
                    std::make_tuple(
                        L"Hello \x4E16\x754C \xD83D\xDE0A!",
                        "Hello \xE4\xB8\x96\xE7\x95\x8C \xF0\x9F\x98\x8A!"
                    ),
            ...
            table< const wchar_t*, const char* >(
                {
                    ...
                    // U+10042 = 𐁂
                    NARROWING_TEST_STR( "\U00010042" ),
                    // U+1F602 = 😂
                    NARROWING_TEST_STR( "\U0001F602" ),
                    // U+24B62 = 𤭢
                    NARROWING_TEST_STR( "\U00024B62" ),
...
TEST_CASE( "util: Widening narrow string to std::wstring", "[stringutil][widen]" ) {
    ...
    SECTION( "Converting UTF-8 strings containing byte sequences encoding UTF-16 surrogates (invalid in UTF-8)" ) {
        const std::string testInput = GENERATE(
            // Lone high surrogates (invalid both in UTF-8 and UTF-16).
            "\xED\xA0\x80",
            "\xED\xA0\x81",
            "\xED\xA0\xBD",
            "\xED\xAA\x80",
            "\xED\xAF\xBE",
            "\xED\xAF\xBF",
            // Lone low surrogates (invalid both in UTF-8 and UTF-16).
            "\xED\xB0\x80",
            "\xED\xB0\x81",
            "\xED\xB2\x80",
            "\xED\xB8\x82",
    ...
    SECTION( "Converting UTF-8 strings containing invalid byte sequences" ) {
        ...
        std::tie( testInput, testOutput ) = GENERATE(
            table< const char*, const wchar_t* >(
                {
                    // Invalid UTF-8 bytes.
                    std::make_tuple( "\xC0", L"\uFFFD" ),
                    std::make_tuple( "\xC1", L"\uFFFD" ),
                    // Bytes form 0xF5 to 0xFF would encode 4-bytes sequences
                    // for codepoints outside the Unicode range, hence they are invalid.
                    ...
                    // Many continuation bytes without a leading byte.
                    std::make_tuple(
                        "\x80\x81\x9A\xA0\xA1\xAF\xB9\xBE\xBF",
                        L"\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD"
                    ),
            ...
            table< const char*, const wchar_t* >(
                {
                    ...
                    //   + invalid UTF-8 byte.
                    std::make_tuple( "\xC3\xC0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xC3\xC1", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xC3\xF5", L"\uFFFD\uFFFD" ),
                    //   + another truncated 2-bytes sequence.
                    std::make_tuple( "\xC3\xC2", L"\uFFFD\uFFFD" ),
                    //   + a valid 2-bytes sequence.
                    std::make_tuple( "\xC3\xC2\xA0", L"\uFFFD\xA0" ),
                    //   + a truncated 3-bytes sequence.
                    std::make_tuple( "\xC3\xE2", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xC3\xE2\x81", L"\uFFFD\uFFFD" ),
                    //   + a valid 3-bytes sequence.
                    std::make_tuple( "\xC3\xE2\x81\x82", L"\uFFFD\u2042" ),
                    //   + a truncated 4-bytes sequence.
                    std::make_tuple( "\xC3\xF0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xC3\xF0\x90", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xC3\xF0\x90\x8D", L"\uFFFD\uFFFD" ),
                    //   + a valid 4-bytes sequence.
                    std::make_tuple( "\xC3\xF0\x90\x8D\x88", L"\uFFFD\xD800\xDF48" ),
                    // Truncated 3-bytes sequence (first byte).
                    std::make_tuple( "\xE2", L"\uFFFD" ),
                    //   + invalid UTF-8 byte.
                    std::make_tuple( "\xE2\xC0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\xC1", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\xF5", L"\uFFFD\uFFFD" ),
                    //   + a truncated 2-bytes sequence.
                    std::make_tuple( "\xE2\xC3", L"\uFFFD\uFFFD" ),
                    //   + a valid 2-bytes sequence.
                    std::make_tuple( "\xE2\xC3\xA9", L"\uFFFD\xE9" ),
                    //   + a truncated 3-bytes sequence.
                    std::make_tuple( "\xE2\xE4", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\xE4\x82", L"\uFFFD\uFFFD" ),
                    //   + a valid 3-bytes sequence.
                    std::make_tuple( "\xE2\xE4\x82\x96", L"\uFFFD\u4096" ),
                    //   + a truncated 4-bytes sequence.
                    std::make_tuple( "\xE2\xF0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\xF0\x90", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\xF0\x90\x8D", L"\uFFFD\uFFFD" ),
                    //   + a valid 4-bytes sequence (𐍈, U+10348).
                    std::make_tuple( "\xE2\xF0\x90\x8D\x88", L"\uFFFD\xD800\xDF48" ),
                    // Truncated 3-bytes sequence (first two bytes).
                    std::make_tuple( "\xE2\x81", L"\uFFFD" ),
                    //   + invalid UTF-8 byte.
                    std::make_tuple( "\xE2\x81\xC0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\x81\xC1", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\x81\xF5", L"\uFFFD\uFFFD" ),
                    //   + a truncated 2-bytes sequence.
                    std::make_tuple( "\xE2\x81\xC3", L"\uFFFD\uFFFD" ),
                    //   + a valid 2-bytes sequence.
                    std::make_tuple( "\xE2\x81\xC3\xA9", L"\uFFFD\xE9" ),
                    //   + a truncated 3-bytes sequence.
                    std::make_tuple( "\xE2\x81\xE4", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\x81\xE4\x82", L"\uFFFD\uFFFD" ),
                    //   + a valid 3-bytes sequence.
                    std::make_tuple( "\xE2\x81\xE4\x82\x96", L"\uFFFD\u4096" ),
                    //   + a truncated 4-bytes sequence.
                    std::make_tuple( "\xE2\x81\xF0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\x81\xF0\x90", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xE2\x81\xF0\x90\x8D", L"\uFFFD\uFFFD" ),
                    //   + a valid 4-bytes sequence (𐍈, U+10348).
                    std::make_tuple( "\xE2\x81\xF0\x90\x8D\x88", L"\uFFFD\xD800\xDF48" ),
                    // Truncated 4-bytes sequence (first byte).
                    std::make_tuple( "\xF0", L"\uFFFD" ),
                    //   + invalid UTF-8 byte.
                    std::make_tuple( "\xF0\xC0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\xC1", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\xF5", L"\uFFFD\uFFFD" ),
                    //   + a truncated 2-bytes sequence.
                    std::make_tuple( "\xF0\xD0", L"\uFFFD\uFFFD" ),
                    //   + a valid 2-bytes sequence (U+0400).
                    std::make_tuple( "\xF0\xD0\x80", L"\uFFFD\u0400" ),
                    //   + a truncated 3-bytes sequence.
                    std::make_tuple( "\xF0\xE0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\xE0\xB8", L"\uFFFD\uFFFD" ),
                    //   + a valid 3-bytes sequence.
                    std::make_tuple( "\xF0\xE0\xB8\xA1", L"\uFFFD\u0E21" ),
                    //   + a truncated 4-bytes sequence.
                    std::make_tuple( "\xF0\xF3", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\xF3\xA0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\xF3\xA0\x80", L"\uFFFD\uFFFD" ),
                    //   + a valid 4-bytes sequence (U+E0022).
                    std::make_tuple( "\xF0\xF3\xA0\x80\xA2", L"\uFFFD\xDB40\xDC22" ),
                    // Truncated 4-bytes sequence (first two bytes).
                    std::make_tuple( "\xF0\x90", L"\uFFFD" ),
                    //   + invalid UTF-8 byte.
                    std::make_tuple( "\xF0\x90\xC0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\xC1", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\xF5", L"\uFFFD\uFFFD" ),
                    //   + a truncated 2-bytes sequence.
                    std::make_tuple( "\xF0\x90\xC4", L"\uFFFD\uFFFD" ),
                    //   + a valid 2-bytes sequence (U+0127).
                    std::make_tuple( "\xF0\x90\xC4\xA7", L"\uFFFD\u0127" ),
                    //   + a truncated 3-bytes sequence.
                    std::make_tuple( "\xF0\x90\xE1", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\xE1\x82", L"\uFFFD\uFFFD" ),
                    //   + a valid 3-bytes sequence.
                    std::make_tuple( "\xF0\x90\xE1\x82\xB4", L"\uFFFD\u10B4" ),
                    //   + a truncated 4-bytes sequence.
                    std::make_tuple( "\xF0\x90\xF0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\xF0\x90", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\xF0\x90\x8D", L"\uFFFD\uFFFD" ),
                    //   + a valid 4-bytes sequence (𐍈, U+10348).
                    std::make_tuple( "\xF0\x90\xF0\x90\x8D\x88", L"\uFFFD\xD800\xDF48" ),
                    // Truncated 4-bytes sequence (first three bytes).
                    std::make_tuple( "\xF0\x90\x8D", L"\uFFFD" ),
                    //   + invalid UTF-8 byte.
                    std::make_tuple( "\xF0\x90\x8D\xC0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\x8D\xC1", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\x8D\xF5", L"\uFFFD\uFFFD" ),
                    //   + a truncated 2-bytes sequence.
                    std::make_tuple( "\xF0\x90\x8D\xC6", L"\uFFFD\uFFFD" ),
                    //   + a valid 2-bytes sequence (U+0400).
                    std::make_tuple( "\xF0\x90\x8D\xC6\x90", L"\uFFFD\u0190" ),
                    //   + a truncated 3-bytes sequence.
                    std::make_tuple( "\xF0\x90\x8D\xEA", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\x8D\xEA\xA7", L"\uFFFD\uFFFD" ),
                    //   + a valid 3-bytes sequence.
                    std::make_tuple( "\xF0\x90\x8D\xEA\xA7\x9E", L"\uFFFD\uA9DE" ),
                    //   + a truncated 4-bytes sequence.
                    std::make_tuple( "\xF0\x90\x8D\xF0", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\x8D\xF0\x90", L"\uFFFD\uFFFD" ),
                    std::make_tuple( "\xF0\x90\x8D\xF0\x90\x8D", L"\uFFFD\uFFFD" ),
                    //   + a valid 4-bytes sequence (𐍈, U+10348).
                    std::make_tuple( "\xF0\x90\x8D\xF0\x90\x8D\x88", L"\uFFFD\xD800\xDF48" ),
            ...
            table< const char*, const wchar_t* >(
                {
                    ...
                    // Overlong sequence for non-ASCII character U+00A2.
                    std::make_tuple( "\xE0\x82\xA2", L"\uFFFD" ),
                    ...
                    // Overlong sequence for non-ASCII character U+00A2.
                    std::make_tuple( "\xF0\x80\x82\xA2", L"\uFFFD" ),
                    // Overlong sequence for non-ASCII character U+20AC (€).
                    std::make_tuple( "\xF0\x82\x82\xAC", L"\uFFFD" ),
                    ...
                    std::make_tuple(
                        "\xFC\x80\x80\x80\x80\x80\xFC\x80\x80\x80\x80\x80",
                        L"\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD"
                    ),
                    // Multiple mixed overlong sequences.
                    std::make_tuple( "\xC0\x80\xE0\x80\x80", L"\uFFFD\uFFFD" ),
                    std::make_tuple(
                        "\xF0\x80\x80\x80\xF8\x80\x80\x80\x80\xF8\x80\x80\x80\x80",
                        L"\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD"
                    )
                }
            )
            ...
            table< const char*, const wchar_t* >(
                {
                    ...
                    WIDENING_TEST_STR( "\u4E08" ),
                    // 世界
                    WIDENING_TEST_STR( "\u4E16\u754C" ),
                    WIDENING_TEST_STR( "\uE000" ),
                    // Replacement character U+FFFD
                    WIDENING_TEST_STR( "\uFFFD" ),
                    // メタルガルルモン
                    WIDENING_TEST_STR( "\u30e1\u30bf\u30eb\u30ac\u30eb\u30eb\u30e2\u30f3" ),
            ...
            table< const char*, const wchar_t* >(
                {
                    ...
                    // U+110001
                    std::make_tuple( "\xF4\x90\x80\x81", L"\uFFFD" ),
                    // U+001FFFFF
                    std::make_tuple( "\xF7\xBF\xBF\xBF", L"\uFFFD" ),
                    // U+00200000
                    std::make_tuple( "\xF8\x88\x80\x80\x80", L"\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD" ),
                    // U+03FFFFFF
                    std::make_tuple( "\xFB\xBF\xBF\xBF\xBF", L"\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD" ),
                    // U+04000000
                    std::make_tuple( "\xFC\x84\x80\x80\x80\x80", L"\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD" ),
                    // U+7FFFFFFF
                    std::make_tuple( "\xFD\xBF\xBF\xBF\xBF\xBF", L"\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD" ),
